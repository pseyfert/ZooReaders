/**
 * @file logstream.cc		implementation of a class for logging output
...
namespace logstreams {
      ...
      const std::string& name)
  {
    ...
    if (!streams.get()) streams = std::auto_ptr<StreamMap>(new StreamMap);
  ...
  void logstream::unregisterStream(logstream* log, bool doDelete)
  {
    ...
    const StreamMapKey key(log->stream, log->myLogLevel, log->myname);
  ...
  std::auto_ptr<logstream::StreamMap> logstream::streams;
